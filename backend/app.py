...
# 模块导入时立刻建表（而不是等到 __main__）。
# 为什么必须放这里：
#   gunicorn 的启动方式是 `app:app`，它是「导入」这个模块来拿 app 对象的，
#   导入时 __name__ 是 "app" 而不是 "__main__"，底下那个 if 分支根本不会执行 ——
#   结果就是容器里 users 表从来没被创建过，所有注册/登录接口全部 500。
#   放到模块级之后，无论 python app.py 还是 gunicorn 启动，都会先建好表。
# 重复执行安全吗？安全 —— init_db() 用的都是 CREATE ... IF NOT EXISTS。
init_db()
...
# ⚠️ 入口必须放在文件最末尾！否则服务器一启动就卡住，后面的函数（如 generate_token）根本来不及定义
if __name__ == "__main__":#判断当前文件是不是被直接运行的。如果是，才执行下面的代码。
    # 注意：init_db() 已经挪到上面「模块级」执行了，这里不用再调
